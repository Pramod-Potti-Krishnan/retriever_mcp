...
def check_connection_status():
    ...
    try:
        ...
        try:
            ...
            embeddings = supabase.table("document_embeddings").select("*", count="exact").execute()
        ...
        with col2:
            if st.button("List Embeddings"):
                try:
                    ...
                    if result.data:
                        st.success(f"Found embeddings (showing first 10)")

                        for emb in result.data:
                            with st.expander(f"Chunk {emb.get('chunk_index', 'Unknown')}"):
                                st.write(emb.get('chunk_text', 'No text'))
                    else:
